...
public class ConferenceCallActivity extends AppCompatActivity implements AlertDialogHelper.AlertDialogListener, OnPageChangeListener, OnLoadCompleteListener, OnPageErrorListener {
    ...
    static final String TAG = "conferenceCallActivity";
    public static Context conferenceCallContext;
    public static Activity conferenceCallActivity;
        ...
        public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
            switch (item.getItemId()) {
                ...
                case R.id.action_reply:
                    ...
                    Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext)).executeTransaction(realm -> {
                        RealmUser realmUser = Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext))
                                .where(RealmUser.class)
                                .equalTo("userid", realmChat.getSenderid())
                                .findFirst();
                        if (realmUser.getRole().equals("student")) {
                            RealmStudent realmStudent = Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext))
                            ...
                            RealmInstructor realmInstructor = Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext))
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        conferenceCallContext = getApplicationContext();
        conferenceCallActivity = this;
        ...
        mLinkPreviewCallback = new LinkPreviewCallback() {
            ...
            public void onPos(SourceContent sourceContent, boolean b) {
                ...
                if (sourceContent.getImages() != null && sourceContent.getImages().size() > 0 && sourceContent.getImages().get(0).toLowerCase().startsWith("http")) {
                    ...
                    Glide.with(conferenceCallContext).
                            load(linkimgurl)
                            .into(linkImage);
        ...
        chat.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (currentIconConstantState.equals(removeIconConstantState)) {
                    ...
                    if (objects.size() == 0 && isNetworkAvailable(conferenceCallContext)) {
                        Toast toast = Toast.makeText(conferenceCallContext, "Checking for new chats...", Toast.LENGTH_SHORT);
                        toast.show();
                    }
                } else {
        ...
        mLinearLayoutManager = new LinearLayoutManager(conferenceCallContext);
        ...
        recyrlerView_participant.setLayoutManager(new LinearLayoutManager(conferenceCallContext));
        ...
        loc.setOnClickListener(v -> {
            try {
                ...
                Intent intent = intentBuilder.build(ConferenceCallActivity.this);
        ...
        sendButton.setOnClickListener(view -> {
            ...
            if (text.length() > 0) {
                ...
                RealmChat realmChat = new RealmChat(
                        ...
                        0,
                        INSTRUCTORCOURSEID,
                        PreferenceManager.getDefaultSharedPreferences(conferenceCallContext).getString(MYUSERID, ""),
                        null,
                        null,
                        null
                );
        ...
        pdf_canvas.setLayoutParams(new FrameLayout.LayoutParams(widthPixels, (int) (heightPixels - convertDpToPx(conferenceCallContext, 200))));
        ...
        pdf.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (docFile == null) {
                    Toast.makeText(ConferenceCallActivity.this, "There is currently no edited document file for this class session.", Toast.LENGTH_SHORT).show();
                } else {
                    ...
                    File pdfBackgoundFile = new File(conferenceCallContext.getFilesDir() + "/SchoolDirectStudent/PDF", "PDF-temp" + ".jpg");
                    ...
                    }, conferenceCallActivity, Uri.fromFile(docFile), pdfPageNum, pdfBackgoundFile).execute();
                }
            }
        });
        ...
        connectTimer.schedule(new TimerTask() {
            @Override
            public void run() {
                conferenceCallActivity.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        try {
                            JSONObject jsonData = new JSONObject()
                                    .put(
                                            "connected_call", PreferenceManager.getDefaultSharedPreferences(conferenceCallContext).getString(MYUSERID, "")
                                    );
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
                    ...
                    case Activity.RESULT_OK:
                        ...
                        if (mMediaSelectedList != null) {
                            ...
                            for (MediaItem mediaItem : mMediaSelectedList) {
                                String path = mediaItem.getPathOrigin(conferenceCallContext);
                                ...
                                if (file.length() > 10000000L) {
                                    ...
                                    if (isExternalStorageWritable()) {
                                        ...
                                        File file_dir = new File(conferenceCallContext.getFilesDir() + "/SchoolDirectStudent/" + COURSEPATH.replace(" >> ", "/") + "/Chats/" + folder, "Sent");
                                        if (!file_dir.exists()) {
                                            file_dir.mkdirs();
                                        }
                                        ...
                                        saveTempChatToRealm(realmChat);
                                        new SendChatAttachmentAsyncTask(conferenceCallContext, realmChat).execute();
                                    }
                                }
                            }
                        } else {
                    ...
                    case Activity.RESULT_OK:
                        if (data != null) {
                            ...
                            for (final Uri selectedDocUri : docUris) {
                                ...
                                try {
                                    filePath = ContentUriUtils.INSTANCE.getFilePath(conferenceCallContext, selectedDocUri);
            ...
            case RC_PLACE_PICKER:
                switch (resultCode) {
                    case Activity.RESULT_OK:
                        Place place = PlacePicker.getPlace(conferenceCallContext, data);
            ...
            case RC_AUDIO_PICKER:
                switch (resultCode) {
                    case Activity.RESULT_OK:
                                    ...
                                    public void apply(final long executionId, final int returnCode) {
                                        if (returnCode == RETURN_CODE_SUCCESS) {
                                            ...
                                            if (isExternalStorageWritable()) {
                                                ...
                                                saveTempChatToRealm(realmChat);
                                                new ConferenceCallActivity.SendChatAttachmentAsyncTask(conferenceCallContext, realmChat).execute();
                                            }
                        ...
                        break;
                    case Activity.RESULT_CANCELED:
                        Toast.makeText(conferenceCallContext, getString(R.string.audio_recording_cancelled), Toast.LENGTH_SHORT).show();
                        break;
                }
                break;
            ...
            if (multiselect_list.size() > 0) {
                ...
                Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext)).executeTransaction(realm -> {
                    for (int i = 0; i < multiselect_list.size(); i++) {
                        RealmChat tempRealmChat = Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext))
...
//            }
    ...
    public void recAudioClicked() {
        ...
        if (EasyPermissions.hasPermissions(this, perms)) {
            ...
            int requestCode = 0;
            AndroidAudioRecorder.with(ConferenceCallActivity.this)
                    // Required
        ...
        try {
            ...
            JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(
                ...
                public Map getHeaders() throws AuthFailureError {
                    ...
                    headers.put("Authorization", "Bearer " + PreferenceManager.getDefaultSharedPreferences(conferenceCallContext).getString(APITOKEN, ""));
                    return headers;
                }
            };
...
//                Log.d("56876", EntityUtils.toString(httpEntity));
    ...
    public void init() {
        ...
        objects.clear();
        populateObjects(conferenceCallContext);
                        ...
                        try {
                            ...
                            switch (jsonObject.getInt("t")) {
                                ...
                                case 7:
                                    ...
                                    Realm.init(conferenceCallContext);
                                    JSONObject finalJsonResponse = jsonResponse;
                                    if (finalJsonResponse.getJSONObject("data").has("connected_call")) {
                                        Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext)).executeTransaction(realm -> {
                                            try {
                                    ...
                                    } else if (finalJsonResponse.getJSONObject("data").has("disconnected_call")) {
                                        Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext)).executeTransaction(realm -> {
                                            try {
                                            ...
                                            try {
                                                ...
                                                if (finalJsonResponse.getJSONObject("data").has("referenced_chat")) {
                                                    ...
                                                    if (realmReferencedChat != null) {
                                                        ...
                                                        if (!realmReferencedChat.getSenderid().equals(PreferenceManager.getDefaultSharedPreferences(conferenceCallContext).getString(MYUSERID, ""))) {
                                                            ...
                                                            realmChat.setReplyname(conferenceCallContext.getString(R.string.me));
                                                        }
                                                    }
                                                }
                                        ...
                                        try {
                                            JSONObject jsonData = new JSONObject()
                                                    ...
                                                    .put(
                                                            "accesstoken", PreferenceManager.getDefaultSharedPreferences(conferenceCallActivity).getString(ACCESSTOKEN, "")
                                                    )
                                                    .put(
                                                            "type", "call"
                                                    )
                                                    ...
                                                    .put(
                                                            "studentid", PreferenceManager.getDefaultSharedPreferences(conferenceCallActivity).getString(MYUSERID, "")
                                                    );
                                            broadcastWithSocket(jsonData.toString());
                                        } catch (JSONException e) {
                                            e.printStackTrace();
...
//                                        sessionupdated = false;
                ...
                Realm.init(conferenceCallContext);
                Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext)).executeTransaction(realm -> {
                    RealmChat realmChat = Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext))
                            .where(RealmChat.class)
...
//                                        broadcastWithSocket(jsonData.toString());
                                    ...
                                    participants.clear();
                                    populateParticipants(conferenceCallContext);
                                    participants.addAll(newParticipants);
                        ...
                        error -> {
                            ...
                            myVolleyError(conferenceCallContext, error);
                        }
                ) {
                    @Override
                    public Map<String, String> getParams() throws AuthFailureError {
                        ...
                        params.put("columntobeupdated", "connectedtocall");
                        params.put("id", id[0]);
                        return params;
                    }
    ...
    public static void drawOnPad(JSONObject dataJsonObject) throws JSONException {
        ...
        if (!dataJsonObject.isNull("background")) {
            ...
            if (background.contains("graph")) {
                pad_canvas.setBackground(conferenceCallContext.getResources().getDrawable(R.drawable.trygraph));
            } else {
    ...
    public static void addMsgToChat(JSONObject finalJsonResponse, RealmChat realmChat) throws JSONException, ParseException {
        ...
        String formattedDate = getFormattedDate(conferenceCallContext, time);
        ...
        if (objects.size() == 0) {
            ...
            if (prevDateString != null) {
                ...
                prevFormattedDate = getFormattedDate(conferenceCallContext, prevTime);
            }
    ...
    public void populateObjects(Context context) {
        ...
        Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext)).executeTransaction(realm -> {
            ...
            for (int i = 0; i < results.size(); i++) {
                ...
                RealmUser realmUser = Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext)).where(RealmUser.class).equalTo("userid", realmChat.getSenderid()).findFirst();
                if (realmUser == null) {
                    continue;
                }
                if (realmUser.getRole().equals("student")) {
                    RealmStudent realmStudent = Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext)).where(RealmStudent.class).equalTo("infoid", realmChat.getSenderid()).findFirst();
                    ...
                    realmChat.setInstructor(true);
                    RealmInstructor realmInstructor = Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext)).where(RealmInstructor.class).equalTo("infoid", realmChat.getSenderid()).findFirst();
            ...
            Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext)).executeTransaction(realm -> {
                ...
                for (RealmChat realmChat : results) {
                    ...
                    if (realmChat.getAttachmenturl() != null && realmChat.getAttachmenturl().startsWith("URI")) {
                        new SendChatAttachmentAsyncTask(conferenceCallContext, recreatedRealChat).execute();
                    } else {
                        try {
    ...
    public void saveTempChatToRealm(RealmChat realmChat) {
        Realm.init(conferenceCallContext);
        Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext)).executeTransaction(realm -> {
            setChatRefParams(realmChat, conferenceCallContext);
            ...
            if (realmChat.getAttachmenturl() != null) {
                Toast.makeText(conferenceCallContext, conferenceCallContext.getString(R.string.uploading_file), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    public static void setChatRefParams(RealmChat realmChat, Context context) {
        if (realmChat.getChatrefid() != null) {
            RealmChat realmReferencedChat = Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext)).where(RealmChat.class).equalTo("chatid", realmChat.getChatrefid()).findFirst();
            realmChat.setReplybody(realmReferencedChat.getText() != null ? realmReferencedChat.getText() : realmReferencedChat.getAttachmenttitle());

            if (!realmReferencedChat.getSenderid().equals(PreferenceManager.getDefaultSharedPreferences(context).getString(MYUSERID, ""))) {
                RealmUser referencedChatUser = Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext))
                        .where(RealmUser.class)
                ...
                if (referencedChatUser.getRole().equals("student")) {
                    realmStudent = Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext))
                            .where(RealmStudent.class)
                            .equalTo("infoid", realmReferencedChat.getSenderid())
                            .findFirst();
                } else if (referencedChatUser.getRole().equals("instructor")) {
                    realmInstructor = Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext))
                            .where(RealmInstructor.class)
                            .equalTo("infoid", realmReferencedChat.getSenderid())
                            .findFirst();
                }
                RealmUser realmUser = Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext)).where(RealmUser.class).equalTo("userid", realmReferencedChat.getSenderid()).findFirst();
                if (realmStudent != null) {
                    realmChat.setReplyname(realmStudent.getFirstname() != null ? realmStudent.getFirstname() : realmUser.getPhonenumber());
                } else if (realmInstructor != null) {
                    realmChat.setReplyname(realmInstructor.getFirstname() != null ? realmInstructor.getFirstname() : realmUser.getPhonenumber());
    ...
    public void broadcastWithSocket(String result) {
        if (isNetworkAvailable(conferenceCallContext)) {
    ...
    public class broadcastWithFirebase extends AsyncTask<Void, Integer, String> {
        ...
        protected String doInBackground(Void... params) {
            ...
            Request request = new Request.Builder()
                    ...
                    .addHeader("Authorization", "Bearer " + PreferenceManager.getDefaultSharedPreferences(conferenceCallActivity).getString(ACCESSTOKEN, ""))
                    .build();
            try {
    ...
    public void populateParticipants(Context context) {
        ...
        Realm.getInstance(RealmUtility.getDefaultConfig(conferenceCallContext)).executeTransaction(realm -> {
            ...
            newParticipants.add(new Participant(realmInstructor.getInfoid(), StringUtils.normalizeSpace((realmInstructor.getTitle() + " " + realmInstructor.getFirstname() + " " + realmInstructor.getOthername() + " " + realmInstructor.getLastname()).replace("null", "")), realmInstructor.getProfilepicurl(), true, realmInstructorCourse.getConnectedtocall()));
            ...
            for (RealmEnrolment realmEnrolment : results) {
                ...
                newParticipants.add(new Participant(realmStudent.getInfoid(), StringUtils.normalizeSpace((realmStudent.getTitle() + " " + realmStudent.getFirstname() + " " + realmStudent.getOthername() + " " + realmStudent.getLastname()).replace("null", "")), realmStudent.getProfilepicurl(), false, realmEnrolment.getConnectedtocall()));
            }
        });
    }
    ...
    public void updateParticipantConnectionStatus(JSONObject finalJsonResponse, Realm realm, int connected) throws JSONException {
        ...
        if (connected == 1) {
            connected_type = "connected_call";
        } else {
            connected_type = "disconnected_call";
        }
        ...
        if (role.equals("student")) {
            ...
            realmEnrolment.setConnectedtocall(connected);
            RealmStudent realmStudent = realm.where(RealmStudent.class).equalTo("infoid", realmEnrolment.getStudentid()).findFirst();
            newParticipants.add(new Participant(realmStudent.getInfoid(), StringUtils.normalizeSpace((realmStudent.getTitle() + " " + realmStudent.getFirstname() + " " + realmStudent.getOthername() + " " + realmStudent.getLastname()).replace("null", "")), realmStudent.getProfilepicurl(), false, realmEnrolment.getConnectedtocall()));
            for (int i = 0; i < participants.size(); i++) {
                if (participants.get(i).getParticipantid().equals(realmStudent.getInfoid())) {
                    participants.set(i, new Participant(realmStudent.getInfoid(), StringUtils.normalizeSpace((realmStudent.getTitle() + " " + realmStudent.getFirstname() + " " + realmStudent.getOthername() + " " + realmStudent.getLastname()).replace("null", "")), realmStudent.getProfilepicurl(), false, realmEnrolment.getConnectedtocall()));
            ...
            realmInstructorCourse.setConnectedtocall(connected);
            RealmInstructor realmInstructor = realm.where(RealmInstructor.class).equalTo("infoid", realmInstructorCourse.getInstructorid()).findFirst();
            for (int i = 0; i < participants.size(); i++) {
                if (participants.get(i).getParticipantid().equals(realmInstructor.getInfoid())) {
                    participants.set(i, new Participant(realmInstructor.getInfoid(), StringUtils.normalizeSpace((realmInstructor.getTitle() + " " + realmInstructor.getFirstname() + " " + realmInstructor.getOthername() + " " + realmInstructor.getLastname()).replace("null", "")), realmInstructor.getProfilepicurl(), true, realmInstructorCourse.getConnectedtocall()));
                    participantsAdapter.notifyItemChanged(i);
                    break;
                }
            }
