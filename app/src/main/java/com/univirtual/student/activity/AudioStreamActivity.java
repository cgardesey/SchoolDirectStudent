...
public class AudioStreamActivity extends AppCompatActivity implements AlertDialogHelper.AlertDialogListener, OnPageChangeListener, OnLoadCompleteListener, OnPageErrorListener {
    ...
    static final String TAG = "audioStreamActivity";
    public static Context audioStreamContext;
    public static Activity audioStreamActivity;
        ...
        public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
            switch (item.getItemId()) {
                ...
                case R.id.action_reply:
                    ...
                    Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext)).executeTransaction(realm -> {
                        RealmUser realmUser = Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext))
                                .where(RealmUser.class)
                                .equalTo("userid", realmChat.getSenderid())
                                .findFirst();
                        if (realmUser.getRole().equals("student")) {
                            RealmStudent realmStudent = Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext))
                            ...
                            RealmInstructor realmInstructor = Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext))
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        audioStreamContext = getApplicationContext();
        audioStreamActivity = this;
                ...
                if (ISRECORDED) {
                    ...
                    if (!filedownloadinprogress) {
                        StringRequest stringRequest = new StringRequest(
                                ...
                                API_URL + "class-audio-session-refresh-data",
                                response -> {
                                    if (response != null) {
                                        try {
                                            JSONObject jsonObject = new JSONObject(response);
                                            Realm.init(audioStreamContext);
                                            Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext)).executeTransaction(new Realm.Transaction() {
                                                @Override
                                            ...
                                            if (jsonObject.getBoolean("eligible")) {
                                                if (jsonObject.getBoolean("eligible")) {
                                                    if (jsonObject.getBoolean("session_changed")) {
                                                        ...
                                                        JSONObject class_audio_session = jsonObject.getJSONObject("class_audio_session");

                                                        SESSIONID = class_audio_session.getString("sessionid");
                                                        AUDIO_URL = class_audio_session.getString("streamurl");
                                                        String docurl = class_audio_session.getString("docurl");
                                                        DIALCODE = class_audio_session.getString("dialcode");
                                                        CONFERENCEID = class_audio_session.getString("roomid");
                                                ...
                                                Toast.makeText(audioStreamContext, getString(R.string.your_subscription_has_expired), Toast.LENGTH_LONG).show();
                                                startActivity(new Intent(audioStreamContext, HomeActivity.class).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP));
                                                finish();
                                            }
                                        } catch (JSONException e) {
                                            e.printStackTrace();
                            ...
                            public Map getHeaders() throws AuthFailureError {
                                ...
                                headers.put("Authorization", "Bearer " + PreferenceManager.getDefaultSharedPreferences(audioStreamContext).getString(APITOKEN, ""));
                                return headers;
                            }
                        };
        ...
        chat.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (currentIconConstantState.equals(removeIconConstantState)) {
                    ...
                    if (objects.size() == 0 && isNetworkAvailable(audioStreamContext)) {
                        Toast toast = Toast.makeText(audioStreamContext, "Checking for new chats...", Toast.LENGTH_SHORT);
                        toast.show();
                    }
                } else {
        ...
        mLinearLayoutManager = new LinearLayoutManager(audioStreamContext);
        ...
        recyrlerView_participant.setLayoutManager(new LinearLayoutManager(audioStreamContext));
        ...
        loc.setOnClickListener(v -> {
            try {
                ...
                Intent intent = intentBuilder.build(AudioStreamActivity.this);
        ...
        sendButton.setOnClickListener(view -> {
            ...
            if (text.length() > 0) {
                ...
                RealmChat realmChat = new RealmChat(
                        ...
                        0,
                        INSTRUCTORCOURSEID,
                        PreferenceManager.getDefaultSharedPreferences(audioStreamContext).getString(MYUSERID, ""),
                        null,
                        null,
                        null
                );
        ...
        pdf_canvas.setLayoutParams(new FrameLayout.LayoutParams(widthPixels, (int) (heightPixels - convertDpToPx(audioStreamContext, 200))));
        ...
        pdf.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (docFile == null) {
                    Toast.makeText(AudioStreamActivity.this, "There is currently no edited document file for this class session.", Toast.LENGTH_SHORT).show();
                } else {
                    ...
                    File pdfBackgoundFile = new File(audioStreamContext.getFilesDir() + "/SchoolDirectStudent/PDF", "PDF-temp" + ".jpg");
                    ...
                    }, audioStreamActivity, Uri.fromFile(docFile), pdfPageNum, pdfBackgoundFile).execute();
                }
            }
        });
        ...
        connectTimer.schedule(new TimerTask() {
            @Override
            public void run() {
                audioStreamActivity.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        try {
                            JSONObject jsonData = new JSONObject()
                                    .put(
                                            "connected_audio", PreferenceManager.getDefaultSharedPreferences(audioStreamContext).getString(MYUSERID, "")
                                    );
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
                    ...
                    case Activity.RESULT_OK:
                        ...
                        if (mMediaSelectedList != null) {
                            ...
                            for (MediaItem mediaItem : mMediaSelectedList) {
                                String path = mediaItem.getPathOrigin(audioStreamContext);
                                ...
                                if (file.length() > 10000000L) {
                                    ...
                                    if (isExternalStorageWritable()) {
                                        ...
                                        File file_dir = new File(audioStreamContext.getFilesDir() + "/SchoolDirectStudent/" + COURSEPATH.replace(" >> ", "/") + "/Chats/" + folder, "Sent");
                                        if (!file_dir.exists()) {
                                            file_dir.mkdirs();
                                        }
                                        ...
                                        saveTempChatToRealm(realmChat);
                                        new SendChatAttachmentAsyncTask(audioStreamContext, realmChat).execute();
                                    }
                                }
                            }
                        } else {
                    ...
                    case Activity.RESULT_OK:
                        if (data != null) {
                            ...
                            for (final Uri selectedDocUri : docUris) {
                                ...
                                try {
                                    filePath = ContentUriUtils.INSTANCE.getFilePath(audioStreamContext, selectedDocUri);
            ...
            case RC_PLACE_PICKER:
                switch (resultCode) {
                    case Activity.RESULT_OK:
                        Place place = PlacePicker.getPlace(audioStreamContext, data);
            ...
            case RC_AUDIO_PICKER:
                switch (resultCode) {
                    case Activity.RESULT_OK:
                                    ...
                                    public void apply(final long executionId, final int returnCode) {
                                        if (returnCode == RETURN_CODE_SUCCESS) {
                                            ...
                                            if (isExternalStorageWritable()) {
                                                ...
                                                saveTempChatToRealm(realmChat);
                                                new AudioStreamActivity.SendChatAttachmentAsyncTask(audioStreamContext, realmChat).execute();
                                            }
                        ...
                        break;
                    case Activity.RESULT_CANCELED:
                        Toast.makeText(audioStreamContext, getString(R.string.audio_recording_cancelled), Toast.LENGTH_SHORT).show();
                        break;
                }
                break;
            ...
            if (multiselect_list.size() > 0) {
                ...
                Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext)).executeTransaction(realm -> {
                    for (int i = 0; i < multiselect_list.size(); i++) {
                        RealmChat tempRealmChat = Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext))
...
//            }
    ...
    public void recAudioClicked() {
        ...
        if (EasyPermissions.hasPermissions(this, perms)) {
            ...
            int requestCode = 0;
            AndroidAudioRecorder.with(AudioStreamActivity.this)
                    // Required
    ...
    public static void initSearchView1(final ArrayList<Object> searchchats, final ChatAdapter chatAdapter) {
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextChange(String newText) {
                ...
                // chatAdapter = new MessageAdapter(audioStreamContext, R.layout.item_message, filteredModelList, mUserId);
...
//                Log.d("56876", EntityUtils.toString(httpEntity));
    ...
    public void init() {
        ...
        objects.clear();
        populateObjects(audioStreamContext);
                        ...
                        try {
                            ...
                            switch (jsonObject.getInt("t")) {
                                ...
                                case 7:
                                    ...
                                    if (finalJsonResponse.getJSONObject("data").has("connected_audio")) {
                                        Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext)).executeTransaction(realm -> {
                                            try {
                                    ...
                                    } else if (finalJsonResponse.getJSONObject("data").has("disconnected_audio")) {
                                        Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext)).executeTransaction(realm -> {
                                            try {
                                            ...
                                            try {
                                                ...
                                                if (finalJsonResponse.getJSONObject("data").has("referenced_chat")) {
                                                    ...
                                                    if (realmReferencedChat != null) {
                                                        ...
                                                        if (!realmReferencedChat.getSenderid().equals(PreferenceManager.getDefaultSharedPreferences(audioStreamContext).getString(MYUSERID, ""))) {
                                                            ...
                                                            realmChat.setReplyname(audioStreamContext.getString(R.string.me));
                                                        }
                                                    }
                                                }
                                        ...
                                        try {
                                            JSONObject jsonData = new JSONObject()
                                                    ...
                                                    .put(
                                                            "accesstoken", PreferenceManager.getDefaultSharedPreferences(audioStreamActivity).getString(ACCESSTOKEN, "")
                                                    )
                                                    .put(
                                                            "type", "audio"
                                                    )
                                                    ...
                                                    .put(
                                                            "studentid", PreferenceManager.getDefaultSharedPreferences(audioStreamActivity).getString(MYUSERID, "")
                                                    );
                                            broadcastWithSocket(jsonData.toString());
                                        } catch (JSONException e) {
                                            e.printStackTrace();
...
//                                        sessionupdated = false;
                ...
                Realm.init(audioStreamContext);
                Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext)).executeTransaction(realm -> {
                    RealmChat realmChat = Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext))
                            .where(RealmChat.class)
...
//                                        broadcastWithSocket(jsonData.toString());
                                    ...
                                    participants.clear();
                                    populateParticipants(audioStreamContext);
                                    participants.addAll(newParticipants);
                        ...
                        error -> {
                            error.printStackTrace();
                            Log.d("Cyrilll", error.toString());
                            myVolleyError(audioStreamContext, error);
                        }
                ) {
                    @Override
                    public Map<String, String> getParams() throws AuthFailureError {
                        Map<String, String> params = new HashMap<>();
                        params.put("instructorcourseid", INSTRUCTORCOURSEID);
                        params.put("columntobeupdated", "connectedtoaudio");
                        params.put("id", id[0]);
                        return params;
                    }
    ...
    public static void drawOnPad(JSONObject dataJsonObject) throws JSONException {
        ...
        if (!dataJsonObject.isNull("background")) {
            ...
            if (background.contains("graph")) {
                pad_canvas.setBackground(audioStreamContext.getResources().getDrawable(R.drawable.trygraph));
            } else {
    ...
    public static void addMsgToChat(JSONObject finalJsonResponse, RealmChat realmChat) throws JSONException, ParseException {
        ...
        String formattedDate = getFormattedDate(audioStreamContext, time);
        ...
        if (objects.size() == 0) {
            ...
            if (prevDateString != null) {
                ...
                prevFormattedDate = getFormattedDate(audioStreamContext, prevTime);
            }
    ...
    public void populateObjects(Context context) {
        ...
        Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext)).executeTransaction(realm -> {
            ...
            for (int i = 0; i < results.size(); i++) {
                ...
                RealmUser realmUser = Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext)).where(RealmUser.class).equalTo("userid", realmChat.getSenderid()).findFirst();
                if (realmUser == null) {
                    continue;
                }
                if (realmUser.getRole().equals("student")) {
                    RealmStudent realmStudent = Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext)).where(RealmStudent.class).equalTo("infoid", realmChat.getSenderid()).findFirst();
                    ...
                    realmChat.setInstructor(true);
                    RealmInstructor realmInstructor = Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext)).where(RealmInstructor.class).equalTo("infoid", realmChat.getSenderid()).findFirst();
            ...
            Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext)).executeTransaction(realm -> {
                ...
                for (RealmChat realmChat : results) {
                    ...
                    if (realmChat.getAttachmenturl() != null && realmChat.getAttachmenturl().startsWith("URI")) {
                        new SendChatAttachmentAsyncTask(audioStreamContext, recreatedRealChat).execute();
                    } else {
                        try {
    ...
    public void saveTempChatToRealm(RealmChat realmChat) {
        Realm.init(audioStreamContext);
        Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext)).executeTransaction(realm -> {
            setChatRefParams(realmChat, audioStreamContext);
            ...
            if (realmChat.getAttachmenturl() != null) {
                Toast.makeText(audioStreamContext, audioStreamContext.getString(R.string.uploading_file), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    public static void setChatRefParams(RealmChat realmChat, Context context) {
        if (realmChat.getChatrefid() != null) {
            RealmChat realmReferencedChat = Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext)).where(RealmChat.class).equalTo("chatid", realmChat.getChatrefid()).findFirst();
            realmChat.setReplybody(realmReferencedChat.getText() != null ? realmReferencedChat.getText() : realmReferencedChat.getAttachmenttitle());

            if (!realmReferencedChat.getSenderid().equals(PreferenceManager.getDefaultSharedPreferences(context).getString(MYUSERID, ""))) {
                RealmUser referencedChatUser = Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext))
                        .where(RealmUser.class)
                ...
                if (referencedChatUser.getRole().equals("student")) {
                    realmStudent = Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext))
                            .where(RealmStudent.class)
                            .equalTo("infoid", realmReferencedChat.getSenderid())
                            .findFirst();
                } else if (referencedChatUser.getRole().equals("instructor")) {
                    realmInstructor = Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext))
                            .where(RealmInstructor.class)
                            .equalTo("infoid", realmReferencedChat.getSenderid())
                            .findFirst();
                }
                RealmUser realmUser = Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext)).where(RealmUser.class).equalTo("userid", realmReferencedChat.getSenderid()).findFirst();
                if (realmStudent != null) {
                    realmChat.setReplyname(realmStudent.getFirstname() != null ? realmStudent.getFirstname() : realmUser.getPhonenumber());
                } else if (realmInstructor != null) {
                    realmChat.setReplyname(realmInstructor.getFirstname() != null ? realmInstructor.getFirstname() : realmUser.getPhonenumber());
    ...
    public void broadcastWithSocket(String result) {
        if (isNetworkAvailable(audioStreamContext)) {
    ...
    public class broadcastWithFirebase extends AsyncTask<Void, Integer, String> {
        ...
        protected String doInBackground(Void... params) {
            ...
            Request request = new Request.Builder()
                    ...
                    .addHeader("Authorization", "Bearer " + PreferenceManager.getDefaultSharedPreferences(audioStreamActivity).getString(ACCESSTOKEN, ""))
                    .build();
            try {
    ...
    public void populateParticipants(Context context) {
        ...
        Realm.getInstance(RealmUtility.getDefaultConfig(audioStreamContext)).executeTransaction(realm -> {
            ...
            newParticipants.add(new Participant(realmInstructor.getInfoid(), StringUtils.normalizeSpace((realmInstructor.getTitle() + " " + realmInstructor.getFirstname() + " " + realmInstructor.getOthername() + " " + realmInstructor.getLastname()).replace("null", "")), realmInstructor.getProfilepicurl(), true, realmInstructorCourse.getConnectedtoaudio()));
            ...
            for (RealmEnrolment realmEnrolment : results) {
                ...
                newParticipants.add(new Participant(realmStudent.getInfoid(), StringUtils.normalizeSpace((realmStudent.getTitle() + " " + realmStudent.getFirstname() + " " + realmStudent.getOthername() + " " + realmStudent.getLastname()).replace("null", "")), realmStudent.getProfilepicurl(), false, realmEnrolment.getConnectedtoaudio()));
            }
        });
    }
    ...
    public void updateParticipantConnectionStatus(JSONObject finalJsonResponse, Realm realm, int connected) throws JSONException {
        ...
        if (connected == 1) {
            connected_type = "connected_audio";
        } else {
            connected_type = "disconnected_audio";
        }
       ...
       if (realmUser == null) {
        ...
        if (role.equals("student")) {
            ...
            realmEnrolment.setConnectedtoaudio(connected);
            RealmStudent realmStudent = realm.where(RealmStudent.class).equalTo("infoid", realmEnrolment.getStudentid()).findFirst();
            newParticipants.add(new Participant(realmStudent.getInfoid(), StringUtils.normalizeSpace((realmStudent.getTitle() + " " + realmStudent.getFirstname() + " " + realmStudent.getOthername() + " " + realmStudent.getLastname()).replace("null", "")), realmStudent.getProfilepicurl(), false, realmEnrolment.getConnectedtoaudio()));
            for (int i = 0; i < participants.size(); i++) {
                if (participants.get(i).getParticipantid().equals(realmStudent.getInfoid())) {
                    participants.set(i, new Participant(realmStudent.getInfoid(), StringUtils.normalizeSpace((realmStudent.getTitle() + " " + realmStudent.getFirstname() + " " + realmStudent.getOthername() + " " + realmStudent.getLastname()).replace("null", "")), realmStudent.getProfilepicurl(), false, realmEnrolment.getConnectedtoaudio()));
            ...
            realmInstructorCourse.setConnectedtoaudio(connected);
            RealmInstructor realmInstructor = realm.where(RealmInstructor.class).equalTo("infoid", realmInstructorCourse.getInstructorid()).findFirst();
            for (int i = 0; i < participants.size(); i++) {
                if (participants.get(i).getParticipantid().equals(realmInstructor.getInfoid())) {
                    participants.set(i, new Participant(realmInstructor.getInfoid(), StringUtils.normalizeSpace((realmInstructor.getTitle() + " " + realmInstructor.getFirstname() + " " + realmInstructor.getOthername() + " " + realmInstructor.getLastname()).replace("null", "")), realmInstructor.getProfilepicurl(), true, realmInstructorCourse.getConnectedtoaudio()));
                    participantsAdapter.notifyItemChanged(i);
                    break;
                }
            }
