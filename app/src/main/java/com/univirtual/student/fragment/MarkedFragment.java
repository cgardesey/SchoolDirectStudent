...
public class MarkedFragment extends Fragment {
    ...
    static ArrayList<RealmAssignment> markedAssignments = new ArrayList<>();
    ...
    public static void populateMarkedAssignments(Context context) {
        ...
        Realm.getInstance(RealmUtility.getDefaultConfig(context)).executeTransaction(realm -> {
            ...
            for (RealmAssignment realmAssignment : results) {
                ...
                if (realmAssignment.getSubmitted() == 1 && realmSubmittedAssignment.getMarkedassignmenturl() != null && !realmSubmittedAssignment.getMarkedassignmenturl().equals("")) {
                    ...
                    realmAssignment.setMarkedassignmenturl(realmSubmittedAssignment.getMarkedassignmenturl());
                    markedAssignments.add(realmAssignment);
                }
            }
        });
    ...
    public static void initMarkedFragment(Context context) {
        populateMarkedAssignments(context);
        if (markedAssignments.size() == 0) {
            nodatatext.setVisibility(View.VISIBLE);
            nodatatext.setText("You currently have no marked assignments \n\nClick on the refresh icon at the top right corner to check for newly marked assignments");
        } else {
            nodatatext.setVisibility(View.GONE);
        }
        GridLayoutManager layoutManager
                = new GridLayoutManager(context, 1, GridLayoutManager.VERTICAL, false);
        AssignmentAdapter assignmentAdapter = new AssignmentAdapter(markedAssignments);
